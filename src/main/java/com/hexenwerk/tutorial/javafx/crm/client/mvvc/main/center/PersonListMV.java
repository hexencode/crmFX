...
public class PersonListMV extends AbstractModelView<List<Person>, PersonListVC> {
    ...
    public void bind(PersonListVC view) {
        ...
        view.tableView.setItems(this.personObservableList);

        FxFlux.from(this.personObservableList).cache()
                .subscribe(change -> change.forEach(personDetailMV -> {
                    System.out.println(personDetailMV.firstNameProperty());
                }));
