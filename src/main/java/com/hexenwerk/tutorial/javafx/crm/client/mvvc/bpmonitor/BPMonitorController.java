...
public class BPMonitorController implements Initializable {
    ...
    private final SortedList<BPData> sortedBackingList =
            new SortedList<>(backingList, Comparator.comparing(BPData::getDate));
    ...
    private final FilteredList<BPData> filteredList =
            new FilteredList<>(sortedBackingList, (BPData p) -> false);

    // Date and Time formatters
    private final DateTimeFormatter formatter =
            DateTimeFormatter.ofPattern("MMM-dd-yy hh:mma");
    private final DateTimeFormatter rangeFormatter =
                ...
                (ListChangeListener.Change<? extends BPData> change) -> {
                    while (change.next()) {
                        ...
                        if (change.wasAdded()) {
                            ...
                            change.getAddedSubList().forEach((BPData data) ->
    ...
    private void initStatChart(XYChart.Series<String, Number> target) {
        target.getData().add(new Data<>("Max", 1));
        target.getData().add(new Data<>("Avg", 1.0));
        target.getData().add(new Data<>("Min", 1));
    ...
    private void addBPDataToChart(BPData data) {
        ...
        // Systolic Series
        Data<String, Number> xyData =
                new Data<>(myDate, data.getSystolic());
        sysData.add(xyData);
        ...
        // Diastolic Series
        xyData = new Data<>(myDate, data.getDiastolic());
        diasData.add(xyData);
        addChartNodeHandler(xyData, sorteddiasData);
        // Pulse Series
        xyData = new Data<>(myDate, data.getPulse());
    ...
    // for this code)
    private void addChartNodeHandler(Data<String, Number> data,
